...
nsp6.on('connection', function(socket) {
  ...
  var sql = "select o_LeakRate from dat_oil_intern where  W_ProgNo=8 and O_ProgNo=7 group by o_LeakRate order by o_LeakRate asc;"+
            ...
            "select W_LeakRate from dat_oil_intern where W_ProgNo=8 and O_ProgNo=7  group by w_LeakRate order by w_LeakRate asc;"+
  ...
  connection.query(sql, function(error, results, fields) {
      ...
      for(var e=0;e<11;e++)
      {
        ...
        {
		t=[];
		maxi=results[e][results[e].length-1];
		// console.log(max);
		mini=results[e][0];
	      	var count1=results[e].length;
	      	if(count1%2!=0)
	 ...
	 else
	 {
	      ...
	      min1["W_LeakRate"]=0;
	      var median11={};
	      median11["W_LeakRate"]=0;
	      var median1={};
	      median1["W_LeakRate"]=0;
	      var median21={};
	      median21["W_LeakRate"]=0;
	      var max1={};
	      max1["W_LeakRate"]=0;
	      resd.push(min1);
	      resd.push(median11);
...
nsp7.on('connection', function(socket) {
        ...
        socket.on('updateGraph7',function(rr){
            add_status3(rr);
        });
        socket.on("end", function(){
          socket.disconnect(0);
...
});/////namespace end for db7

var add_status3 = function (param) {
